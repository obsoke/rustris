...
impl UITextView {
    pub fn new(ctx: &mut Context, assets: &Assets, top_left: Point, title: &str, value: &str) -> Self {
        ...
        Self {
            ...
            top_left: Point::new(top_left.x + (WIDTH / 2.0), top_left.y + (UITEXTVIEW_HEIGHT / 2.0)),
            centre: top_left,
    ...
    pub fn draw(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        // value portion
        let value_rect = Rect::new(self.top_left.x - 20.0, self.top_left.y + UITEXTVIEW_HEIGHT, WIDTH - 40.0, UITEXTVIEW_HEIGHT);
...
impl UIBlockView {
    pub fn new(ctx: &mut Context, assets: &Assets, top_left: Point, title: &str, shape: Option<PieceType>) -> Self {
        let title_txt = graphics::Text::new(ctx, title, assets.get_font("normal").unwrap()).unwrap();
        Self {
            // ggez draws coords at center; we want to use top left as reference
    ...
    pub fn draw(&mut self, ctx: &mut Context, assets: &Assets) -> GameResult<()> {
        ...
        // value portion
        let value_rect = Rect::new(self.top_left.x - 20.0, self.top_left.y + HEIGHT + (HEIGHT / 2.0), WIDTH - 40.0, HEIGHT * 2.0);
        graphics::set_color(ctx, Color::new(1.0, 0.0, 0.0, 1.0))?;
